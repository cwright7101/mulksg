...
import mulksg_init
mulksg_init.init()
mulksg_home = mulksg_init.mulksg_home
bin_home = mulksg_init.bin_home
python_modules_home = mulksg_init.python_modules_home
ext_python_modules_home = mulksg_init.ext_python_modules_home
mulksg_version = mulksg_init.mulksg_version
...
def print_used_values(cfg, log):
    ...
    try:
        log.info("  MULKSG, using SPAdes version: " + str(mulksg_version).strip())
...
def fill_cfg(options_to_parse, log, secondary_filling=False):
    ...
    if options_storage.test_mode:
        if options_storage.plasmid:
            support.add_to_dataset('-1', os.path.join(mulksg_home, "test_dataset_plasmid/pl1.fq.gz"), dataset_data)
            support.add_to_dataset('-2', os.path.join(mulksg_home, "test_dataset_plasmid/pl2.fq.gz"), dataset_data)
        else:
            support.add_to_dataset('-1', os.path.join(mulksg_home, "test_dataset/ecoli_1K_1.fq.gz"), dataset_data)
            support.add_to_dataset('-2', os.path.join(mulksg_home, "test_dataset/ecoli_1K_2.fq.gz"), dataset_data)
...
#            support.error("you cannot specify multiple k-mer sizes in RNA-Seq mode!", log)
    ...
    if not support.get_lib_ids_by_type(dataset_data, mulksg_logic.READS_TYPES_USED_IN_CONSTRUCTION):
        support.error('you should specify at least one unpaired, paired-end, or high-quality mate-pairs library!')
    if options_storage.rna:
        if len(dataset_data) != len(support.get_lib_ids_by_type(dataset_data, mulksg_logic.READS_TYPES_USED_IN_RNA_SEQ)):
            support.error('you cannot specify any data types except ' +
                          ', '.join(mulksg_logic.READS_TYPES_USED_IN_RNA_SEQ) + ' in RNA-Seq mode!')
...
def show_version():
    options_storage.version(mulksg_version)
    sys.exit(0)


def show_usage(code, show_hidden=False):
    options_storage.usage(mulksg_version, show_hidden=show_hidden)
    sys.exit(code)
...
def main(args):
    ...
    if rank == 0:
        ...
        log_filename = os.path.join(cfg["common"].output_dir, "mulksg.log")
        if options_storage.continue_mode:
        ...
        if options_storage.continue_mode:
            log.info("\n======= MULSG pipeline continued. Log can be found here: " + log_filename + "\n")
    ...
    #        del cfg["mismatch_corrector"]
        ...
        if not options_storage.continue_mode:
            log.info("\n======= MULKSG pipeline started (Using SPAdes). Log can be found here: " + log_filename + "\n")
        ...
        try:
            ...
            if not os.path.isdir(tmp_configs_dir):
                if options_storage.configs_dir:
                    ...
                    dir_util.copy_tree(os.path.join(mulksg_home, "configs"), tmp_configs_dir, preserve_times=False, preserve_mode=False)
            ...
            if "assembly" in cfg and not options_storage.run_completed:
                ...
                mulksg_cfg = merge_configs(cfg["assembly"], cfg["common"])
                mulksg_cfg.__dict__["result_contigs"] = result_contigs_filename
                mulksg_cfg.__dict__["result_scaffolds"] = result_scaffolds_filename
                mulksg_cfg.__dict__["result_graph"] = result_assembly_graph_filename
                mulksg_cfg.__dict__["result_graph_gfa"] = result_assembly_graph_filename_gfa
                mulksg_cfg.__dict__["result_contigs_paths"] = result_contigs_paths_filename
                mulksg_cfg.__dict__["result_scaffolds_paths"] = result_scaffolds_paths_filename
                mulksg_cfg.__dict__["result_transcripts"] = result_transcripts_filename
                mulksg_cfg.__dict__["result_transcripts_paths"] = result_transcripts_paths_filename

                if options_storage.continue_mode and (os.path.isfile(mulksg_cfg.result_contigs)
                    ...
                    latest_dir = support.get_latest_dir(os.path.join(mulksg_cfg.output_dir, "K*"))
                    if not latest_dir:
                    ...
                    if mulksg_cfg.disable_rr:
                        mulksg_cfg.__dict__["rr_enable"] = False
                    else:
                        mulksg_cfg.__dict__["rr_enable"] = True

                    if "HEAPCHECK" in os.environ:
                        del os.environ["HEAPCHECK"]
                    if "heap_check" in mulksg_cfg.__dict__:
                        os.environ["HEAPCHECK"] = mulksg_cfg.heap_check

                    log.info("\n===== %s started.\n" % STAGE_NAME)

                    # creating dataset
                    dataset_filename = os.path.join(mulksg_cfg.output_dir, "dataset.info")
                    if not os.path.isfile(dataset_filename) or not options_storage.continue_mode:
                        ...
                        if mulksg_cfg.developer_mode and "reference" in cfg["dataset"].__dict__:
                            dataset_file.write("reference_genome" + '\t')
                            dataset_file.write(process_cfg.process_spaces(cfg["dataset"].reference) + '\n')
                        dataset_file.close()
                    mulksg_cfg.__dict__["dataset"] = dataset_filename
                    assemble_start = time.time()
                    used_K = mulksg_logic.run_mulksg(tmp_configs_dir, bin_home, mulksg_cfg, dataset_data, ext_python_modules_home, log, comm, rank, size)
                    assemble_end = time.time()
                    if rank == 0:
                        with open("AssemblyTiming.log", "a") as log:
                            log.write("\nAssembly time for MULKSG " + dataset_filename+ " is: " + str(assemble_end - assemble_start) + " seconds")
            ...
            if not support.log_warnings(log):
                log.info("\n======= MULKSG (Using SPAdes) pipeline finished.")  # otherwise it finished WITH WARNINGS
            ...
            log.info("\nMULKSG log can be found here: " + log_filename)
            log.info("")
            log.info("Thank you for using MULKSG!")
            ...
            if exc_type == SystemExit:
                ...
                if exc_type == OSError and exc_value.errno == errno.ENOEXEC: # Exec format error
                    support.error("It looks like you are using MULKSG binaries for another platform.\n" +
        ...
        used_K = mulksg_logic.run_mulksg(None, None, None, None, None, log, comm, rank, size)#I don't need any info, just running a command
        # used_K = mulksg_logic.run_mulksg(tmp_configs_dir, bin_home, mulksg_cfg, dataset_data, ext_python_modules_home, log)
