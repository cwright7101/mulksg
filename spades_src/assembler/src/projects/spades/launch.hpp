...
void mulksg_assemble_genome() {
    INFO("MULKSG started");
    if (cfg::get().mode == debruijn_graph::config::pipeline_type::meta && !MetaCompatibleLibraries()) {
        ERROR("Sorry, current version of metaMULKSG can work either with single library (paired-end only) "
                      "or in paired-end + (TSLR or PacBio or Nanopore) mode.");
        exit(239);
    }
    ...
    StageManager MULKSG({cfg::get().developer_mode,
                         cfg::get().load_from,
                         cfg::get().output_saves});
    ...
    // Build the pipeline
    MULKSG.add<debruijn_graph::Construction>();

    if (cfg::get().mode != debruijn_graph::config::pipeline_type::meta)
        MULKSG.add<debruijn_graph::GenomicInfoFiller>();

    VERIFY(!cfg::get().gc.before_raw_simplify || !cfg::get().gc.before_simplify);

    if (cfg::get().gap_closer_enable &&
        cfg::get().gc.before_raw_simplify)
        MULKSG.add<debruijn_graph::GapClosing>("early_gapcloser");

    //Using two_step_rr is hacky here. Fix soon!
    MULKSG.add<debruijn_graph::RawSimplification>(two_step_rr);
    ...
    if (two_step_rr) {
        MULKSG.add<debruijn_graph::Simplification>(true);
        if (cfg::get().gap_closer_enable && cfg::get().gc.after_simplify)
            MULKSG.add<debruijn_graph::GapClosing>("prelim_gapcloser");
        if (cfg::get().use_intermediate_contigs) {
            MULKSG.add<debruijn_graph::PairInfoCount>(true)
                    .add<debruijn_graph::DistanceEstimation>(true)
                    .add<debruijn_graph::RepeatResolution>(true)
                    .add<debruijn_graph::ContigOutput>()
                    .add<debruijn_graph::SecondPhaseSetup>();
    ...
    MULKSG.add<debruijn_graph::Simplification>();

    if (cfg::get().gap_closer_enable && cfg::get().gc.after_simplify)
        MULKSG.add<debruijn_graph::GapClosing>("late_gapcloser");

    MULKSG.add<debruijn_graph::SimplificationCleanup>();

    if (cfg::get().correct_mismatches)
        MULKSG.add<debruijn_graph::MismatchCorrection>();

    if (cfg::get().rr_enable) {
        if (!cfg::get().series_analysis.empty())
            MULKSG.add<debruijn_graph::SeriesAnalysis>();

        if (cfg::get().pd)
            MULKSG.add<debruijn_graph::ChromosomeRemoval>();

        if (HybridLibrariesPresent())
            MULKSG.add<debruijn_graph::HybridLibrariesAligning>();

        //No graph modification allowed after HybridLibrariesAligning stage!

        MULKSG.add<debruijn_graph::ContigOutput>(false, "intermediate_contigs")
               .add<debruijn_graph::PairInfoCount>()
               .add<debruijn_graph::DistanceEstimation>()
               .add<debruijn_graph::RepeatResolution>();
    } else {
        MULKSG.add<debruijn_graph::ContigOutput>(false);
    }

    MULKSG.add<debruijn_graph::ContigOutput>();

    MULKSG.run(conj_gp, cfg::get().entry_point.c_str());

    debruijn_graph::config::write_lib_data(fs::append_path(cfg::get().output_dir, "final"));

    INFO("MULKSG finished");
}
